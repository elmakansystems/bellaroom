...
router.get("/create", isSales, async (req, res) => {
  try {
    const products = await product
      .find({ p_type: "collection" })
      .select("name price");
    const old_products = await product
      .find({ p_type: null })
      .select("name price");
    const parts = await product.find({ p_type: "part" }).select("name price");
    const p_buy = await buy.find();
    ...
    res.status(201).render("invoices/create", {
      products,
      dealers,
      serial,
      parts,
...
router.post("/create_new", isSales, async (req, res) => {
  ...
  products.forEach(async (p, i) => {
    ...
    try {
      await buy.updateOne(
        { _id: products[i].split("|")[3] },
        { $set: { inv_p_count: products[i].split("|")[2] - 1 } }
      );
